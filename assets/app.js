...
/* =========================================================
   DEMO APP — Router, Views, Firestore Models, UI Helpers
   ========================================================= */
...
let demoAlias = localStorage.getItem("aks_demo_alias") || randomAlias();

/* ------------------------------
   Utils
------------------------------ */
function randomAlias(){
  const animals = ["고래","수달","매","사자","여우","매미","비둘기","수리부엉이","돌고래","치타"];
  return `게스트-${animals[Math.floor(Math.random()*animals.length)]}-${Math.floor(Math.random()*1000)}`;
}
...
document.getElementById("signinBtn").addEventListener("click", async ()=>{
  ...
  if(!email || !password){ showToast("이메일/비밀번호를 입력하세요"); return; }
  try{
    await signInWithEmailAndPassword(auth, email, password);
    loginModal.close();
    showToast("로그인 완료");
  }catch(e){ showToast("로그인 실패: " + e.code); }
});
document.getElementById("signupBtn").addEventListener("click", async ()=>{
  ...
  try{
    ...
    loginModal.close();
    showToast("회원가입 완료");
  }catch(e){ showToast("회원가입 실패: " + e.code); }
});
...
const COL = {
  ...
  LEARNING: "learning",     // 러닝 항목(스킬/코스/리소스)
  ANACADEMY: "anacademy",   // 안카데미(러닝 로그)
  IDP: "idp",               // IDP 카탈로그(스킬/직무→추천)
  MYLEARN: "my_learning"    // 내 학습(개인 보관함/진척)
};
...
async function createDoc(col, payload){
...
async function listDocs(col, opts={}){
  ...
  if(filters.length){
    // Firestore는 복수 where 결합 시 복합 인덱스가 필요할 수 있음
    // 에러 발생 시 콘솔 링크로 인덱스 생성
    filters.forEach(f=> q = query(q, where(f[0], f[1], f[2])));
  }
  const snap = await getDocs(q);
  return snap.docs.map(d=>({ id:d.id, ...d.data() }));
}
async function updateField(col, id, patch){
...
async function globalSearchRun(keyword){
  ...
  // 간단 전략: 각 컬렉션에서 최신 N개 가져와 client-side filter (데모 한정)
  const size = 40;
  ...
  const hit = (x)=> {
    const fields = [
      x.title, x.body, x.content, x.note, x.tags?.join(", "),
      (x.searchTokens||[]).join(" ")
...
async function renderIDP(){
  const [catalog, my] = await Promise.all([
    ...
    listDocs(COL.MYLEARN, { lim:50, filters:[["owner","==", currentIdentity()]] }).catch(()=>[]) // rules에 따라 실패 가능
  ]);
  const html = /*html*/`
  ...
  const html = /*html*/`
    ...
    ${list.length? list.map(r=>/*html*/`
      <article class="card">
        ...
        ${r.body||r.content||r.note? `<div style="white-space:pre-wrap; margin-top:6px">${escapeHtml((r.body||r.content||r.note).slice(0,240))}${(r.body||r.content||r.note).length>240?"…":""}</div>`:""}
...
submitPostBtn.addEventListener("click", async (e)=>{
  ...
  const authorName = currentUser?.email || demoAlias;
  const base = {
    title, body, tags, authorName, authorEmail: currentUser?.email||null,
...
async function openEditPost(p){
  // 간단 편집: 기존 값으로 모달 열기 → 저장 시 update
  ...
  submitPostBtn.onclick = async (e)=>{
    ...
    submitPostBtn.onclick = null; // 핸들러 원복
    composerForm.reset();
    composerModal.close();
    showToast("수정 완료");
    render("#home");
...
function currentIdentity(){
  return currentUser?.email || demoAlias;
}
...
/* =========================================================
   Firestore 인덱스 팁(오류 발생 시 콘솔 링크로 생성)
   - posts: orderBy(createdAt desc)
   - notices: orderBy(createdAt desc)
   - learning: orderBy(createdAt desc)
   - anacademy: orderBy(createdAt desc)
   - idp: orderBy(createdAt desc)
   - my_learning: where(owner == <identity>) + orderBy(createdAt desc)  ← 복합 인덱스 필요 가능
========================================================= */
